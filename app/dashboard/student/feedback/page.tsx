...
export default function FeedbackPage() {
  ...
  const previousFeedback = [
    {
      ...
      rating: 5,
      comment: "Excellent course! James is a fantastic instructor who really helped me improve my freestyle technique.",
      date: "March 15, 2023",
    },
    {
      id: 2,
  ...
  return (
    <DashboardLayout userRole="student">
      ...
      <Tabs defaultValue="give-feedback" className="mt-8">
        ...
        <TabsContent value="give-feedback" className="space-y-6 mt-6">
          <h2 className="text-xl font-bold">Courses Available for Feedback</h2>
            ...
            {courses.map((course) => (
              <Card key={course.id}>
                ...
                <CardContent>
                  ...
                  {course.completed && !course.feedbackSubmitted && !submittedFeedback && (
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map((star) => (
                            ...
                            >
                              <Star
                                className={`h-8 w-8 ${
                                  rating && star <= rating ? "text-yellow-400 fill-yellow-400" : "text-muted-foreground"
                                }`}
                              />
                            </button>
                          ))}
                  ...
                  {submittedFeedback && (
                    <div className="bg-green-50 p-4 rounded-lg">
                      <p className="text-center text-green-700">
                        Thank you for your feedback! Your input helps us improve our courses.
                      </p>
                    </div>
                  )}
                ...
                <CardFooter>
                  {course.completed && !course.feedbackSubmitted && !submittedFeedback && (
                    <Button onClick={handleSubmitFeedback} disabled={!rating || !feedback}>
        ...
        <TabsContent value="previous-feedback" className="space-y-6 mt-6">
              ...
              {previousFeedback.map((feedback) => (
                <Card key={feedback.id}>
                  <CardHeader>
                    ...
                    <CardDescription>Instructor: {feedback.instructor}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center gap-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star
                            key={star}
                            className={`h-5 w-5 ${
                              star <= feedback.rating ? "text-yellow-400 fill-yellow-400" : "text-muted-foreground"
                            }`}
                          />
                        ))}
