...
import { Waves, User, BookOpen, Calendar, Award, Clock, CheckCircle2, Bell, CreditCard, FileText } from "lucide-react"
    ...
    notifications: [
      {
        ...
        message: "Your 'Arm Movements for Freestyle' lesson is tomorrow at 4:00 PM",
        date: "May 6, 2023",
        read: false,
      },
      {
                ...
                <CardContent>
                  <div className="grid gap-2 md:grid-cols-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">Location: {lesson.location}</span>
                    </div>
                  </div>
                </CardContent>
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                {user.notifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`flex items-start gap-4 border-b pb-4 ${!notification.read ? "bg-sky-50 p-3 rounded-md" : ""}`}
                  >
                    <Bell
                      className={`h-5 w-5 mt-0.5 ${!notification.read ? "text-sky-500" : "text-muted-foreground"}`}
                    />
                    ...
                    {!notification.read && (
                      <Badge className="bg-sky-100 text-sky-700 hover:bg-sky-200 cursor-pointer">Mark as read</Badge>
                    )}
                  </div>
                ))}
              </div>
