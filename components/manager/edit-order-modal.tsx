...
import { updateOrder, CreateOrderData, Order } from "@/api/manager/orders-api";
...
export function EditOrderModal({
  ...
  // Populate form with order data when modal opens
  useEffect(() => {
    if (open && order) {
      // Set order type
      const type = order.type?.[0] || "member";
      setOrderType(type as OrderType);

      // Set course
      const courseId =
        typeof order.course === "string" ? order.course : order.course?._id;
      ...
      // Set status
      setStatus(order.status?.[0] || "paid");

      // Set price
      setPrice(String(order.price || ""));

      // Set guest data if available
      if (order.guest) {
        setGuestName(order.guest.username || "");
        setGuestPhone(order.guest.phone || "");
        setGuestEmail(order.guest.email || "");
      }
    }
  }, [open, order]);
  ...
  const handleSubmit = async () => {
    ...
    if (selectedCourse && selectedCourse.is_active === false) {
      toast({
        title: "Lỗi",
        description:
          "Không thể cập nhật giao dịch với khóa học không hoạt động",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      await updateOrder(order._id, orderData, tenantId, token);
